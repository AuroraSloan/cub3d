...
{
	while (mm->wid_cnt < mm->mm_wid / (int)info->col_count)
	{
...
void	draw_mini_map(t_info *info)
{
	...
	while (mm.mm_i < mm.mm_hght && mm.map_i < (int)info->row_count)
	{
		mm.hght_cnt = 0;
		while (mm.hght_cnt < mm.mm_hght / (int)info->row_count)
		{
			mm.map_j = 0;
			mm.mm_j = 5;
			while (mm.mm_j < mm.mm_wid && mm.map_j < (int)info->col_count)
			{
				mm.wid_cnt = 0;
				draw_items(info, &mm);
				mm.map_j++;
