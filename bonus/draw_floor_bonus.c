...
void	draw_floor(t_info *info, t_ray *r, int stripe)
{
	...
	while (y < info->hght)
	{
		...
		r->current_floor.x = r->weight * r->floor_wall.x
			+ (1.0 - r->weight) * info->pos.x;
		r->current_floor.y = r->weight * r->floor_wall.y
			+ (1.0 - r->weight) * info->pos.y;
		r->floor_tex.x = (int)(r->current_floor.x * TEX_WID) % TEX_WID;
		r->floor_tex.y = (int)(r->current_floor.y * TEX_HGHT)
			% TEX_HGHT;
		info->buf[y][stripe] = info->texture[5][TEX_WID * r->floor_tex.y
			+ r->floor_tex.x];
		y++;
	}
}
